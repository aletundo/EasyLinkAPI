 ...
 */
public class AnalyzeAllCallable extends AnalyzeCallable<Map<String, List<EasyLinkBean>>> {
    public AnalyzeAllCallable(Language language, String inputText, String scoredCandidates, int threshold, String domain, TaskStateAbstract t) {
        super(language, inputText, scoredCandidates, threshold, domain, t);
    }

    @Override
    public Map<String, List<EasyLinkBean>> buildResults(String inputText) {
        Map<String, List<EasyLinkBean>> resultsMap = new HashMap<>();
            ...
            if (BabelPOS.NOUN.equals(syns.getPOS()) || BabelPOS.ADJECTIVE.equals(syns.getPOS())) {
                ...
                if (e.getTitle() != null && e.getGloss() != null) {
                    if (getDomain() == null || getDomain().trim().equals("") || getDomain().equals("ALL") || getDomain().equalsIgnoreCase(e.getBabelDomain())){
                        if(!resultsMap.containsKey(e.getTitle())){
                            List<EasyLinkBean> availableAnnotations = new ArrayList<>();
                            availableAnnotations.add(e);
                            resultsMap.put(e.getTitle(), availableAnnotations);
                        }else {
                            resultsMap.get(e.getTitle()).add(e);
                        }
                    }
                }
            }
    ...
    @Override
    public Map<String, List<EasyLinkBean>> call() throws Exception {
        Map<String, List<EasyLinkBean>> results = buildResults(getInputText());
